...
- `Assets/Scripts/Brick.cs` — accumulates the ball's `Damage` per hit against its `Hardness` (1–3, set per row by `GameManager`); destroys itself and reports its `Points` to `GameManager` once damage reaches hardness. While still alive it shows a crack overlay on the prefab's `Cracks` child renderer — light crack up to half damage, heavy crack beyond — from sprites wired in by the editor setup script.
