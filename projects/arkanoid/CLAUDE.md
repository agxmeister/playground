...
- `Assets/Scripts/GameManager.cs` — state machine (Ready/Playing/GameOver/Won), score, lives, high score persisted via `PlayerPrefs` (`Arkanoid.HighScore`), runtime brick-grid construction. Pushes values into the `ScoreBoard`; OnGUI is only used for the state banner. Serialized references to the Ball/Brick prefabs, the scene Paddle and the ScoreBoard are wired by the editor setup script.
